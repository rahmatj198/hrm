...
//        echo '<br />';
    ...
    private function readXf()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            switch ($horAlign) {
                case 0:
                    $objStyle->getAlignment()->[7]Horizontal(PHPExcel_Style_Alignment::HORIZONTAL_GENERAL);
                    break;
                case 1:
            ...
            switch ($wrapText) {
                case 0:
                    $objStyle->getAlignment()->[7]WrapText(false);
                    break;
                case 1:
            ...
            switch ($vertAlign) {
                case 0:
                    $objStyle->getAlignment()->s[7]ertical(PHPExcel_Style_Alignment::VERTICAL_TOP);
                    break;
                case 1:
    ...
    private function readFormula()
    {
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            if ((ord($recordData[6]) == 0) && (ord($recordData[12]) == 255) && (ord($recordData[13]) == 255)) {
                ...
                // Boolean formula. Re[7]t is in +2; 0=false, 1=true
                $dataType = PHPExcel_Cell_DataType::TYPE_BOOL;
                $value = (bool) ord($recordData[8]);
            } elseif ((ord($recordData[6]) == 2)
                && (ord($recordData[12]) == 255)
                && (ord($recordData[13]) == 255)) {
                // Error formula. Erro[7]ode is in +2
                $dataType = PHPExcel_Cell_DataType::TYPE_ERROR;
                $value = PHPExcel_Reader_Excel5_ErrorCode::lookup(ord($recordData[8]));
            } elseif ((ord($recordData[6]) == 3)
                && (ord($recordData[12]) == 255)
                && (ord($recordData[13]) == 255)) {
                // Formula result is a[7]ll string
